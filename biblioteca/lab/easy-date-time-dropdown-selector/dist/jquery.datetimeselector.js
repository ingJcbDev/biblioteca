	...
	jQuery.fn.dateTimeSelector = function (options) {
		options = $.extend(
			{
				...
				// Day options
				dayDefault: "Seleccionar dia",
				dayValue: "",
				dayClass: "",

				// Hour options
				hourDefault: "Seleccionar hora",
				hourValue: "",
				hourClass: "",

				// Minute options
				minuteDefault: "Seleccionar minutos",
				minuteValue: "",
				minuteClass: "",

				// Second options
				secondDefault: "Seleccionar segundo",
				secondValue: "",
				secondClass: "",
			},
			options
		...
		var getOption = {
			y: function (value) {
				...
				var result = options.yearDefault
					? '<option value="----">' + options.yearDefault + "</option>"
					: "";
				var selected = "";
			...
			m: function (value) {
				...
				var result = options.monthDefault
					? '<option value="--">' + options.monthDefault + "</option>"
					: "";
				var selected = "";
			...
			d: function (value) {
				...
				var result = options.dayDefault
					? '<option value="--">' + options.dayDefault + "</option>"
					: "";
				var selected = "";
			...
			h: function (value) {
				...
				var result = options.hourDefault
					? '<option value="--">' + options.hourDefault + "</option>"
					: "";
				var selected = "";
			...
			i: function (value) {
				...
				var result = options.minuteDefault
					? '<option value="--">' + options.minuteDefault + "</option>"
					: "";
				var selected = "";
			...
			s: function (value) {
				...
				var result = options.secondDefault
					? '<option value="--">' + options.secondDefault + "</option>"
					: "";
				var selected = "";
		...
		var selectCreate = function (selector) {
			...
			for (var i = 0; i < formatArray.length; i++) {
				...
				if (container == "start") {
					result +=
						"<" +
						options["section" + container_type + "Tag"] +
						' class="' +
						options["section" + container_type + "Class"] +
						'">';
					container = "";
				}
		...
		var getSelectParams = function (id) {
			var re = /dts_(id|name)_(.*)_(y|m|d|h|i|s)_dte/gi,
				match,
				params = {},
				decode = function (s) {
					return s.replace(/\+/g, " ");
