...
if (isset($pointer->status) && !$pointer->status) {
  ...
  if ($pointer->rowCount() !== 0) {
    ...
    while ($data = $pointer->fetch(PDO::FETCH_ASSOC)) {
      ...
      // valida que el indice anterior sea igual al indice actual o que este vacio o igual a empty
      if ($currentIndex == $index || $currentIndex == "empty") {
        //  cuando la validacion se cumple se incrementa
        $accountant++;
      } else {
        // se iguala el contador a uno
        $accountant = 1;
        // se iguala el indice anterior a empty
        $currentIndex = "empty";
      }
      // cantidad de datos obtenidos
      $vars[$index]['length_'] = $accountant;
      // se iguala el indice anterior con el actual
      $currentIndex = $index;
    }
  }
